...
- 개발 전용 `Profiler`를 dev/prod 엔트리로 분리해 운영 번들에서 디버깅 코드 제외
...
답변: 처음에는 컴포넌트 내부에서 PROD 분기로 우회했지만, 그 경우 파일 자체는 번들에 남을 수 있다고 판단했습니다.
그래서 Profiler 구현을 dev/prod 파일로 분리하고, Vite alias를 build 시점에 전환하도록 설정했습니다.
